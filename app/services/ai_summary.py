...
def get_ai_summary(user: User) -> Dict[str, Any]:
    ...
    base_url = user.ai_base_url or os.environ.get("OPENAI_BASE_URL")  # Например: http://localhost:11434/v1 для Ollama
    if not api_key:
        return {"error": "API-ключ не настроен. Добавьте его в Настройках или в .env файл."}

    it_news = get_it_news(limit=5)
    pol_news = get_politics_news(limit=5)
